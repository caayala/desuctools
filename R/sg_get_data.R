...
sg_get_data <- function(
  ...
  if (
    sum(api_operation %in% c("SimpleExport", "OperationData")) !=
      length(api_operation)
  ) {
    stop(
      "\"api_operation\" must be one of: \"SimpleExport\" or \"OperationData\""
    )
  }
  ...
  df_interviews <- data.frame(
    group = ceiling(seq_along(interview_ids) / 99),
