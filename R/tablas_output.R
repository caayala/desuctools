...
tabla_categoria <- function(.df, .var, .wt = NULL, miss = NULL) {
  ...
  if (!is.null(miss)) {
    tab <- tabla_prop_val(tab, miss = c(NA, miss))
  }
...
tabla_prop_val <- function(.df, by = NULL, miss) {
  ...
  .df$casos_val <- replace(.df$casos, .df$pregunta_cat %in% miss, NA)
...
tabla_var_segmento <- function(
  ...
  if (inherits(v_pregunta, c("haven_labelled", "factor", "character"))) {
    ...
    # Agrega el porcentaje válido si es que se señalan categorias perdidas.
    if (!is.null(miss)) {
      tab <- tabla_prop_val(tab, miss = c(NA, miss), by = "segmento_cat")
    }
  } else {
    ...
    tab <- aggregate(
      ...
      FUN = function(idx) {
        ...
        x_val <- c(!is.na(x) & !(x %in% miss))
        w <- df_agg[["casos"]][idx]
        # browser()
        c(
          casos = sum(w, na.rm = TRUE),
          casos_val = sum(w[x_val], na.rm = TRUE),
          mean = weighted.mean(x[x_val], w[x_val], na.rm = TRUE)
        )
      },
      drop = FALSE
    )
...
tabla_vars_segmentos.data.frame <- function(
  ...
  vars <- tidyselect::eval_select(expr = rlang::enquo(.vars), data = .df)
  vars <- names(vars)
  ...
  if (rlang::quo_is_null(segmentos_quo)) {
    ...
    segmentos <- tidyselect::eval_select(expr = segmentos_quo, data = .df)
