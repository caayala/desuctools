...
# Ejecutar la función
v_labelled_rec <- rec_labelled(.vec = v_labelled, ls_niveles = ls_rec)

test_that("Recodificación de etiquetas", {
  expect_equal(
    names(attr(v_labelled_rec, "labels")),
    c(
...
test_that("Recodificación de valores", {
  ...
  expect_equal(as.numeric(v_labelled_rec), c(1, 1, 1, 1, 2, 3, 3, 3, 1, 99))
})
