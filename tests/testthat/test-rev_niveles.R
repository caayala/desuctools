...
test_that("rev_niveles haven_labelled", {
  # Valores sin rango
  expect_equal(
    as.numeric(rev_niveles(var_labelled, rango_inv = NULL)),
    c(9, 8, 8, 7, 1)
  )
  # Etiquetas sin rango
  expect_equal(
    attr(rev_niveles(var_labelled, rango_inv = NULL), "labels"),
    c("a" = 9, "b" = 8, "c" = 7, "nr" = 1)
  )

  # Valores con rango
  expect_equal(
    as.numeric(rev_niveles(var_labelled, rango_inv = c(1, 2))),
    c(2, 1, 1, 3, 9)
  )
  # Etiquetas con rango
  expect_equal(
    attr(rev_niveles(var_labelled, rango_inv = c(1, 2)), "labels"),
    c("a" = 2, "b" = 1, "c" = 3, "nr" = 9)
  )
...
test_that("rev_niveles factor", {
  ...
  expect_equal(
    levels(rev_niveles(var_factor, niveles_inv = c("a", "b"))),
    c("b", "a", "c", "nr")
  )
